...
## We propose a novel hierarchical Bayesian solution fit on observed health district-level counts of new cases

- SEIR compartmental process with detection using a continuous approximation of the binomial distribution (no discrete priors in stan)
  - Lay interpretation: we model the true number of infections as a hidden process that generates the observed case counts through probabilistic detection.
...
```{r betabin_trans,include=T,echo=F,cache=TRUE}
library(tidyverse)
TT=30
N_C=11
...
```{r ar1_trans,include=T,echo=F,cache=TRUE}
library(tidyverse)
library(gganimate)
