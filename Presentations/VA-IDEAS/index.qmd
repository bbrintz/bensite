...
- Smooth logistic change-point ramps capture medium-term structural shifts 
  - Pre-specified number but estimated using horshoe priors to allow for flexibility
...
## Utah 211 call patterns (Covid example)
```{r,include=T,echo=F,cache=TRUE}
library(tidyverse)
library(gganimate)
...
p=yfit %>% filter(date>ymd("2019-09-01")) %>% ggplot(aes(x=date,y=mean, ymin=q5, ymax=q95)) + 
  ...
  geom_rect(data = rect_df %>% filter(mean >= 245, q5 >= 245), inherit.aes = FALSE,
            aes(xmin = dts$date[match(round(q5),dts$dt)], xmax = dts$date[match(round(q95),dts$dt)],
            alpha=vis,group=id), fill="blue",
            ymin = 0, ymax = Inf) + scale_fill_viridis_d(begin=.2,end=.8) + 
            scale_alpha_identity() +
            labs(caption=str_wrap("Red dots are observed call counts. Blue shaded intervals represent 90% credible intervals for change-point timing. Dashed lines indicate mean change-point estimates.", width = 100))  +
  theme_minimal() + xlab("") + 
...
## Probabilistic surge detection (example figure)

![](call_spikes_alertprob_placeholder.png){height=500px}
