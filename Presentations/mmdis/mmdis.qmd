...
theme_bw()+ ggtitle("lambda=0.5")+ scale_fill_viridis_d(begin=.25,end=.75)
...
theme_bw()+ ggtitle("lambda=0.8")+ scale_fill_viridis_d(begin=.25,end=.75)
...
theme_bw()+ ggtitle("lambda=1")+ scale_fill_viridis_d(begin=.25,end=.75)
...
## How can we mitigate the effect of biases on decision making tools? {auto-animate=true auto-animate-duration="3" auto-animate-easing="ease-in-out"}
::: {style="text-decoration: underline;"}
Post-Processing
:::
...
::: {data-id="ROC" auto-animate-delay="3" data-visibility="visible" style="background: #00000; width: 750px; height: auto; margin: 5px;"}
...
## How can we mitigate the effect of biases on decision making tools? {auto-animate=true auto-animate-duration="3" auto-animate-easing="ease-in-out" visibility="uncounted"}
::: {style="text-decoration: underline;"}
Post-Processing
:::
Threshold Optimization for Equalized Odds
...
::: {data-id="ROC" style="background: #00000; width: 750px; height: auto; margin: 5px;"}
```{r,include=T,echo=F,cache=TRUE}
library(pROC)
rocs=c2 %>% split(.$Female) %>% purrr::map(function(x) roc(x$Two_yr_Recidivism,x$probability))  

vline_data <- data.frame(
  xintercept = c(1 - 0.391, 1 - 0.3753),
  Sex = c("Male", "Female")
)

# Data frame for horizontal dashed lines
hline_data <- data.frame(
  yintercept = c(0.851, 0.85),
  Sex = c("Male", "Female")
)
