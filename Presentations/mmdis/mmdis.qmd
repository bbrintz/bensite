...
## How can we mitigate the effect of biases on decision making tools? {.no-incremental .visibility="uncounted"}
::: {style="text-decoration: underline;"}
Regularization-Based
:::
- Tries to minimize the negative log likelihood of the model 
...
log_likelihood <- function(beta, X, Y,A,lam1=1) {
  ...
  pA1=p[which(A==1 & Y==1)]
  pA0=p[which(A==0 & Y==1)]
  pen1=abs(mean(pA1)-mean(pA0))
  logLL=-(sum(Y * log(p) + (1 - Y) * log(1 - p))) + lam1*log(pen1)
  logLL
}
```
