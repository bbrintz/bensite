...
## We approximate the binomial distribution with the inverse logit of a normal distribution
- more efficient sampling and leverages the strength of gradient-based algorithms
...
df_normal <- tibble(
  sample = rep(normal_samples, each=10),
  frame = rep(1:10, each=nrep)
)
df_invlogit <- tibble(
  sample = rep(invlogit_samples, each=10),
  frame = rep(11:20, each=nrep)
)
df_binom <- tibble(
  sample = rep(binom_sim, each=10),
  frame = rep(11:20, each=nrep)
)
...
p2 <- ggplot() +
  geom_density(data = df_normal, aes(x = sample), fill = "orange", alpha = 0.5) +
    geom_density(data = df_invlogit, aes(x = sample), fill = "orange", alpha = 0.5) +

  geom_density(data = df_binom, aes(x = sample), color = "black", linewidth = 1, linetype = "dashed") +
...
# Animate
anim <- animate(p2, nframes = 200, fps = 20)
anim
```
